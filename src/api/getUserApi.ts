...
const getUserApi = {
    getUser: () => {
        const url = 'v1/api/getusers';
        return axiosClient.get(url);
    },
    changeLock: (lock: DataLock) => {
        const data = new FormData();
        const url = 'v1/api/users/change_lock';
    ...
    changeStatus: (status: DataStatus) => {
        const data = new FormData();
        const url = 'v1/api/users/change_status';
    ...
    changeCode: (code: DataCode) => {
        const data = new FormData();
        const url = 'v1/api/users/add_code';
    ...
    removeUser: (id: string) => {
        const data = new FormData();
        const url = 'v1/api/users/delete_user';
        data.append('id', id);
        return axiosClient.post(url, data);
    },
};
